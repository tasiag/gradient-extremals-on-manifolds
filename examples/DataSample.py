...
class DataSample():
    ...
    def __init__(self, name, keep = 0.02, debug_plot = 0, u=False, sigma=None, seed = None):
        ...
        self.FES = f"data/{name}_fes.dat"#_smoothed.dat"
    ...
    def __gather_data_cv_energy_with_spline(self):
        ...
        spline = scipy.interpolate.SmoothBivariateSpline(fes["d1(A)"], fes["d2(A)"], fes["Free Energy(J)"])
        predicted_energy = spline(df["C1"], df["C2"], grid=False)
        ...
        if self.DEBUG_PLOT == 1:
            ...
            a = ax1.scatter(fes["d1(A)"], fes["d2(A)"], s=0.5, c=fes["Free Energy(J)"], vmin= self.vmin, vmax = self.vmax)
            plt.colorbar(a)
